...
int child_handler(PipesCommunication* comm, local_id id);
...
int main(int argc, char** argv)
{
    ...
    {
        child_handler(comm, current_proc_id);
    }
#ifdef DEBUG_MAIN
    fprintf(stdout, "MAIN: 进程 %d 执行完毕。\n", current_proc_id);
#endif
...
#ifdef DEBUG_MAIN
    fprintf(stdout, "CHILD %d: 就绪。\n", id);
#endif
    increment_lamport_time();
    receive_all_msgs(comm, STARTED);
#ifdef DEBUG_MAIN
    fprintf(stdout, "CHILD %d: 接收其他进程%s。\n", id, Name(STARTED));
#endif
    // 发送转账，停止，完成消息
...
int transfer_message(PipesCommunication* comm, Message* msg, BalanceState* state, BalanceHistory* history)
{
    ...
    {
#ifdef DEBUG_MAIN
        fprintf(stdout, "%s : %d payed money\n", Name(transfer_message), current_id);
#endif
    ...
    {
#ifdef DEBUG_MAIN
        fprintf(stdout, "%s : %d received money\n", Name(transfer_message), current_id);
#endif
...
void update_history(BalanceState* state, BalanceHistory* history, balance_t amount, timestamp_t timestamp_msg, char inc, char fix)
{
    ...
    if (amount > 0)
    {
        ...
        {
#ifdef DEBUG_MAIN
            fprintf(stdout, "%s : child %d has $%d amount", Name(update_history), i, amount);
#endif
